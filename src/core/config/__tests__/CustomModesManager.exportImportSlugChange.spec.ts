...
import { getProjectRooDirectoryForCwd } from "../../../services/roo-config" // bluescode_change
...
describe("CustomModesManager", () => {
	...
	const mockRoomodes = path.join(mockWorkspacePath, ".bluescodemodes")
	...
	describe("getCustomModes", () => {
		...
		it("should merge modes with .bluescodemodes taking precedence", async () => {
			const settingsModes = [
			...
			// mode2 should come from .bluescodemodes since it takes precedence
		...
		it("should handle missing .bluescodemodes file", async () => {
			const settingsModes = [{ slug: "mode1", name: "Mode 1", roleDefinition: "Role 1", groups: ["read"] }]
		...
		it("should handle invalid YAML in .bluescodemodes", async () => {
			const settingsModes = [{ slug: "mode1", name: "Mode 1", roleDefinition: "Role 1", groups: ["read"] }]
			...
			// Should fall back to settings modes when .bluescodemodes is invalid
			expect(modes).toHaveLength(1)
			expect(modes[0].slug).toBe("mode1")
		})
	...
	describe("updateCustomMode", () => {
		it("should update mode in settings file while preserving .bluescodemodes precedence", async () => {
			...
			// Should update global state with merged modes where .bluescodemodes takes precedence
			expect(mockContext.globalState.update).toHaveBeenCalledWith(
				"customModes",
				expect.arrayContaining([
					expect.objectContaining({
						slug: "mode1",
						name: "Roomodes Mode 1", // .bluescodemodes version should take precedence
						source: "project",
					}),
				]),
			)
		...
		it("creates .bluescodemodes file when adding project-specific mode", async () => {
			...
			// Mock .bluescodemodes to not exist initially
			...
			// Verify .bluescodemodes was created with the project mode
			...
			// Verify the content written to .bluescodemodes
	...
	describe("updateModesInFile", () => {
		...
		describe("importModeWithRules", () => {
			...
			it("should successfully import mode without rules files", async () => {
				...
				expect(fs.writeFile).toHaveBeenCalledWith(
					expect.stringContaining(".bluescodemodes"),
			...
			it("should prevent path traversal attacks in import", async () => {
				...
				const mockWorkspacePath = path.resolve("/mock/workspace")
				const writtenRuleFiles = writtenFiles.filter((p) => !p.includes(".bluescodemodes"))
				writtenRuleFiles.forEach((filePath) => {
					const normalizedPath = path.normalize(filePath)
					const expectedBasePath = path.normalize(
						getProjectRooDirectoryForCwd(mockWorkspacePath), // bluescode_change
					)
	...
	describe("checkRulesDirectoryHasContent", () => {
		...
		it("should work with global custom modes when .roomodes doesn't exist", async () => {
			...
			;(fs.stat as Mock).mockResolvedValue({ isDirectory: () => true })
			;(fs.readdir as Mock).mockResolvedValue([
				{ name: "rule1.md", isFile: () => true, parentPath: "/mock/workspace/.bluescode/rules-test-mode" },
			])
	...
	describe("exportModeWithRules", () => {
		...
		it("should successfully export mode with rules for a built-in mode customized in .roomodes", async () => {
			...
			;(fs.stat as Mock).mockResolvedValue({ isDirectory: () => true })
			;(fs.readdir as Mock).mockResolvedValue([
				{ name: "rule1.md", isFile: () => true, parentPath: "/mock/workspace/.bluescode/rules-code" },
			])
