...
describe("RooIgnoreController", () => {
	...
	describe("initialization", () => {
		/**
		 * Tests the controller initialization when .bluescodeignore exists
		 */
		it("should load .bluescodeignore patterns on initialization when file exists", async () => {
			// Setup mocks to simulate existing .bluescodeignore file
			...
			expect(mockFileExists).toHaveBeenCalledWith(path.join(TEST_CWD, ".bluescodeignore"))
			expect(mockReadFile).toHaveBeenCalledWith(path.join(TEST_CWD, ".bluescodeignore"), "utf8")
		...
		/**
		 * Tests the controller behavior when .bluescodeignore doesn't exist
		 */
		it("should allow all access when .bluescodeignore doesn't exist", async () => {
			// Setup mocks to simulate missing .bluescodeignore file
		 ...
		 */
		it("should set up file watcher for .bluescodeignore changes", async () => {
			// Check that watcher was created with correct pattern
			expect(vscode.workspace.createFileSystemWatcher).toHaveBeenCalledWith(
				expect.objectContaining({
					base: TEST_CWD,
					pattern: ".bluescodeignore",
				}),
			)
		 ...
		 */
		it("should handle errors when loading .bluescodeignore", async () => {
			...
			expect(consoleSpy).toHaveBeenCalledWith("Unexpected error loading .bluescodeignore:", expect.any(Error))
	...
	describe("validateAccess", () => {
		...
		/**
		 * Tests the default behavior when no .bluescodeignore exists
		 */
		it("should allow all access when no .bluescodeignore content", async () => {
			// Create a new controller with no .bluescodeignore
			mockFileExists.mockResolvedValue(false)
	...
	describe("validateCommand", () => {
		...
		/**
		 * Tests behavior when no .bluescodeignore exists
		 */
		it("should allow all commands when no .bluescodeignore exists", async () => {
			// Create a new controller with no .bluescodeignore
			mockFileExists.mockResolvedValue(false)
			const emptyController = new RooIgnoreController(TEST_CWD)
			await emptyController.initialize()
	...
	describe("getInstructions", () => {
		/**
		 * Tests instructions generation with .bluescodeignore
		 */
		it("should generate formatted instructions when .bluescodeignore exists", async () => {
			// Setup .bluescodeignore content
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules\n.git\nsecrets/**")
			await controller.initialize()
	...
	describe("file watcher", () => {
		/**
		 * Tests behavior when .bluescodeignore is created
		 */
		it("should reload .bluescodeignore when file is created", async () => {
			// Setup initial state without .bluescodeignore
			mockFileExists.mockResolvedValue(false)
			await controller.initialize()
			...
			// Create and initialize controller with no .bluescodeignore
			controller = new RooIgnoreController(TEST_CWD)
			await controller.initialize()
			...
			// Force reload of .bluescodeignore content manually
			await controller.initialize()
		...
		/**
		 * Tests behavior when .bluescodeignore is changed
		 */
		it("should reload .bluescodeignore when file is changed", async () => {
			// Setup initial state with .bluescodeignore
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules")
			await controller.initialize()
		...
		/**
		 * Tests behavior when .bluescodeignore is deleted
		 */
		it("should reset when .bluescodeignore is deleted", async () => {
			// Setup initial state with .bluescodeignore
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules")
			await controller.initialize()
