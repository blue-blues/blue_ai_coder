...
export function getSystemPromptFilePath(cwd: string, mode: Mode): string {
	// bluescode_change
	return path.join(cwd, ".bluescode", `system-prompt-${mode}`)
}

/**
 * Loads custom system prompt from a file at .bluescode/system-prompt-[mode slug]
...
/**
 * Ensures the .bluescode directory exists, creating it if necessary
 */
export async function ensureRooDirectory(cwd: string): Promise<void> {
	// bluescode_change
	const rooDir = path.join(cwd, ".bluescode")
