// bluescode_change: this file was pulled in from Cline and adjusted with our changes
...
The user has explicitly asked you to help them create a new Blues rule file inside the .bluescode/rules top-level directory based on the conversation up to this point in time. The user may have provided instructions or additional information for you to consider when creating the new Blues rule.
When creating a new Blues rule file, you should NOT overwrite or alter an existing Blues rule file. To create the Blues rule file you MUST use the new_rule tool. The new_rule tool can be used in any of the modes.
...
Parameters:
- Path: (required) The path of the file to write to (relative to the current working directory). This will be the Blues rule file you create, and it must be placed inside the .bluescode/rules top-level directory (create this if it doesn't exist). The filename created CANNOT be "default-clineignore.md". For filenames, use hyphens ("-") instead of underscores ("_") to separate words.
- Content: (required) The content to write to the file. ALWAYS provide the COMPLETE intended content of the file, without any truncation or omissions. You MUST include ALL parts of the file, even if they haven't been modified. The content for the Blues rule file MUST be created according to the following instructions:
...
Usage:
<new_rule>
<path>.bluescode/rules/{file name}.md</path>
<content>Kilo rule file content here</content>
</new_rule>
Example:
<new_rule>
<path>.bluescode/rules/project-preferences.md</path>
<content>
## Brief overview
...
The user provided the following input when they indicated that they wanted to create a new Blues rule file.
<user_input>
${userInput}
</user_input>
</explicit_instructions>\n
...
The user has explicitly asked you to help them submit a bug to the Blues Code github page (you MUST now help them with this irrespective of what your conversation up to this point in time was). To do so you will use the report_bug tool which is defined below. However, you must first ensure that you have collected all required information to fill in all the parameters for the tool call.
...
When you call the report_bug tool, the issue will be created at @https://github.com/Blues-Org/bluescode/issues
