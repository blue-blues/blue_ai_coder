...
const resendMessageSequence = async (
	...
	if (!currentCline || currentCline.taskId !== taskId) {
		...
		vscode.window.showErrorMessage(t("bluescode:userFeedback.message_update_failed"))
		return false
	}
	...
	try {
		...
		if (revert) {
			...
			if (checkpointMessage && checkpointMessage.text) {
				...
				vscode.window.showErrorMessage(t("bluescode:userFeedback.no_checkpoint_found"))
			}
		}
		...
		if (success) {
			vscode.window.showInformationMessage(t("bluescode:userFeedback.message_updated"))
		}
	} catch (error) {
