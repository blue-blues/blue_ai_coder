...
import type { ModeConfig, PromptComponent } from "@blues-code/types"
...
describe("isToolAllowedForMode", () => {
	...
	describe("file restrictions", () => {
		...
		it("handles partial streaming cases (path only, no content/diff)", () => {
			...
			// Should allow path-only for code mode too
			expect(
				isToolAllowedForMode("write_to_file", "code", [], undefined, {
					path: "test.js",
				}),
			).toBe(true)
		})
		...
		it("allows code mode to edit all file types", () => {
			// Should allow editing any file type in code mode
			expect(
				isToolAllowedForMode("write_to_file", "code", [], undefined, {
					path: "test.md",
					content: "# Test",
			...
			// Should allow applying diffs to any file type
			expect(
				isToolAllowedForMode("apply_diff", "code", [], undefined, {
					path: "readme.md",
					diff: "- old\n+ new",
				}),
			).toBe(true)
			...
			// Should maintain all capabilities
			expect(isToolAllowedForMode("read_file", "code", [])).toBe(true)
			expect(isToolAllowedForMode("browser_action", "code", [])).toBe(true)
			expect(isToolAllowedForMode("use_mcp_tool", "code", [])).toBe(true)
		})

		it("allows code mode to use all edit tools on any file type", () => {
			// Test search_and_replace with any file type
			expect(
				isToolAllowedForMode("search_and_replace", "code", [], undefined, {
					path: "test.md",
					search: "old text",
					replace: "new text",
				}),
			...
			// Test insert_content with any file type
			expect(
				isToolAllowedForMode("insert_content", "code", [], undefined, {
					path: "test.md",
					line: "1",
					content: "# New content",
				}),
		...
		it("applies restrictions to apply_diff with concurrent file edits (MULTI_FILE_APPLY_DIFF experiment)", () => {
			...
			// Should allow all files in code mode
			expect(
				isToolAllowedForMode("apply_diff", "code", [], undefined, {
					args: xmlArgs,
				}),
			...
			// Test with python file - should be allowed in code mode
			const xmlArgsNonMd =
			...
			// Test with multiple files - should allow all file types in code mode
			...
			// Test with mixed file types - should allow all in code mode
			const xmlArgsMixed =
...
describe("FileRestrictionError", () => {
	...
	describe("code mode", () => {
		it("is configured correctly", () => {
			...
			expect(codeMode).toMatchObject({
				slug: "code",
				name: "💻 Code", // bluescode_change
				roleDefinition:
					"You are Kilo Code, a highly skilled software engineer with extensive knowledge in many programming languages, frameworks, design patterns, and best practices.",
				groups: ["read", "edit", "browser", "command", "mcp"],
			})
			expect(codeMode?.customInstructions).toContain(
	...
	describe("getFullModeDetails", () => {
		...
		it("applies custom mode overrides", async () => {
			...
			const result = await getFullModeDetails("code", customModes)
			expect(result).toMatchObject({
				slug: "code",
				name: "Custom Code",
				roleDefinition: "Custom code role",
		...
		it("applies prompt component overrides", async () => {
			...
			const result = await getFullModeDetails("code", undefined, customModePrompts)
		...
		it("combines custom instructions when cwd provided", async () => {
			...
			await getFullModeDetails("code", undefined, undefined, options)
		...
		it("falls back to first mode for non-existent mode", async () => {
			...
			expect(result).toMatchObject({
				...modes[0],
				// The first mode (code) has its own customInstructions
			})
		})
	})
...
describe("getModeSelection", () => {
	...
	const promptComponentOrchestrator: PromptComponent = {
		roleDefinition: "Prompt Component Orchestrator Role",
		customInstructions: "Prompt Component Orchestrator Instructions",
	}
	...
	test("should prioritize promptComponent for built-in mode if no custom mode exists for that slug", () => {
		const selection = getModeSelection("orchestrator", promptComponentOrchestrator) // "orchestrator" is not in customModesList
		expect(selection.roleDefinition).toBe(promptComponentOrchestrator.roleDefinition)
		expect(selection.baseInstructions).toBe(promptComponentOrchestrator.customInstructions)
	})
	...
	test("promptComponent is used if customMode for slug does not exist, even if customModesList is provided", () => {
		// 'orchestrator' is not in customModesList, but 'code' and 'new-custom' are.
		const selection = getModeSelection("orchestrator", promptComponentOrchestrator, customModesList)
		expect(selection.roleDefinition).toBe(promptComponentOrchestrator.roleDefinition)
		expect(selection.baseInstructions).toBe(promptComponentOrchestrator.customInstructions)
	})

	test("builtInMode is used if customMode for slug does not exist and promptComponent is not provided", () => {
		// 'orchestrator' is not in customModesList
		const selection = getModeSelection("orchestrator", undefined, customModesList)
		const builtInOrchestratorMode = modes.find((m) => m.slug === "orchestrator")!
		expect(selection.roleDefinition).toBe(builtInOrchestratorMode.roleDefinition)
		expect(selection.baseInstructions).toBe(builtInOrchestratorMode.customInstructions || "")
	})

	test("promptComponent is used if customMode is not provided (undefined customModesList)", () => {
		const selection = getModeSelection("orchestrator", promptComponentOrchestrator, undefined)
		expect(selection.roleDefinition).toBe(promptComponentOrchestrator.roleDefinition)
		expect(selection.baseInstructions).toBe(promptComponentOrchestrator.customInstructions)
	})
})
