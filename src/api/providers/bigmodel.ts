...
export class BigModelHandler extends BaseProvider implements SingleCompletionHandler {
	...
	constructor(options: ApiHandlerOptions) {
		...
		this.client = new OpenAI({
			...
			apiKey: this.options.bigModelApiKey ?? "not-provided",
	...
	override getModel() {
		const id =
			this.options.apiModelId && this.options.apiModelId in bigModelModels
				? (this.options.apiModelId as BigModelModelId)
				: bigModelDefaultModelId

		const info = bigModelModels[id]
	...
	override async *createMessage(
		...
		const stream = await this.client.chat.completions.create({
			...
			temperature: this.options.modelTemperature ?? BIGMODEL_DEFAULT_TEMPERATURE,
	...
	async completePrompt(prompt: string): Promise<string> {
		...
		try {
			...
			if (error instanceof Error) {
				throw new Error(`bigModel completion error: ${error.message}`)
			}
