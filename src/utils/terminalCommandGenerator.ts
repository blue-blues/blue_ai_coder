...
export async function generateTerminalCommand(options: TerminalCommandGeneratorOptions): Promise<void> {
	...
	try {
		...
		if (!activeTerminal) {
			vscode.window.showErrorMessage(t("bluescode:terminalCommandGenerator.noActiveTerminal"))
			return
		}
...
async function showWarningIfNeeded(context: vscode.ExtensionContext): Promise<boolean> {
	...
	if (!warningAcknowledged) {
		const warningChoice = await vscode.window.showInformationMessage(
			t("bluescode:terminalCommandGenerator.warningDialog.title"),
			{ modal: true, detail: t("bluescode:terminalCommandGenerator.warningDialog.message") },
			t("bluescode:terminalCommandGenerator.warningDialog.okButton"),
			t("bluescode:terminalCommandGenerator.warningDialog.cancelButton"),
		)

		if (warningChoice !== t("bluescode:terminalCommandGenerator.warningDialog.okButton")) {
			return false
		}
...
async function getUserInput(): Promise<string | undefined> {
	return await vscode.window.showInputBox({
		prompt: t("bluescode:terminalCommandGenerator.inputPrompt"),
		placeHolder: t("bluescode:terminalCommandGenerator.inputPlaceholder"),
		ignoreFocusOut: true,
	})
}
	...
	await vscode.window.withProgress(
		{
			...
			title: t("bluescode:terminalCommandGenerator.generatingProgress"),
			cancellable: false,
		},
		async () => {
			...
			vscode.window.showInformationMessage(
				t("bluescode:terminalCommandGenerator.commandGenerated", { command: cleanCommand }),
			)
		},
	)
}
