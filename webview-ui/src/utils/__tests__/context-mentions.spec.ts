...
describe("getContextMenuOptions", () => {
	...
	it("should return all option types for empty query", () => {
		...
		expect(result).toHaveLength(7) // bluescode_change: added image option
		expect(result.map((item) => item.type)).toEqual([
			...
			ContextMenuOptionType.Image, // bluescode_change
	...
	it("should process slash commands when both query and inputValue start with slash", () => {
		const mockModes = [
			...
			},
			{
				slug: "orchestrator",
				name: "🪃 Orchestrator",
				roleDefinition: "You are an orchestrator assistant",
				groups: ["read" as const, "edit" as const],
			},
		]
