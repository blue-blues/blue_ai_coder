...
import { type TokenUsage, BluesCodeEventName, taskEventSchema } from "@blues-code/types"
import type { Run, Task, TaskMetrics } from "@blues-code/evals"
...
export const useRunStatus = (run: Run): RunStatus => {
	...
	const onMessage = useCallback((messageEvent: MessageEvent) => {
		...
		switch (eventName) {
			case BluesCodeEventName.TaskStarted:
				startTimes.current.set(taskId, Date.now())
				break
			case BluesCodeEventName.TaskTokenUsageUpdated: {
			...
			case BluesCodeEventName.EvalPass:
			case BluesCodeEventName.EvalFail:
