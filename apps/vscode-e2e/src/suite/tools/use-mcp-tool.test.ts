...
import { BluesCodeEventName, type ClineMessage } from "@blues-code/types"
...
suite.skip("Roo Code use_mcp_tool Tool", function () {
	...
	suiteSetup(async () => {
		...
		testFiles = {
			...
			mcpConfig: path.join(workspaceDir, ".bluescode", "mcp.json"),
		}
		...
		const rooDir = path.join(workspaceDir, ".bluescode")
	...
	test("Should request MCP filesystem read_file tool and complete successfully", async function () {
		...
		api.on(BluesCodeEventName.Message, messageHandler)
		...
		api.on(BluesCodeEventName.TaskStarted, taskStartedHandler)
		...
		api.on(BluesCodeEventName.TaskCompleted, taskCompletedHandler)
		...
		try {
			...
			// Clean up
			api.off(BluesCodeEventName.Message, messageHandler)
			api.off(BluesCodeEventName.TaskStarted, taskStartedHandler)
			api.off(BluesCodeEventName.TaskCompleted, taskCompletedHandler)
		}
	})
