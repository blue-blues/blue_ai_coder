...
suite("Kilo Code Modes", function () {
	...
	test("Should handle switching modes correctly", async () => {
		...
		globalThis.api.on(BluesCodeEventName.TaskModeSwitched, (_taskId, mode) => modes.push(mode))
		...
		const switchModesTaskId = await globalThis.api.startNewTask({
			...
			text: "Use the `switch_mode` tool to switch to `orchestrator` mode.",
		})

		await waitUntilCompleted({ api: globalThis.api, taskId: switchModesTaskId })
		await globalThis.api.cancelCurrentTask()

		assert.ok(modes.includes("orchestrator"))
